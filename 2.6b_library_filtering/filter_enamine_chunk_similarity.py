...
python filter_enamine_chunk_similarity.py \
    26900 \
    /pi/summer.thyme-umw/enamine-REAL-2.6billion \
    /path/to/output/chunk_26900 \
    --similarity-cutoff 0.75 \
    --max-ligands 50000

If automatic directory discovery does not match the library layout, provide:
    --chunk-dir /exact/path/to/the/chunk/directory
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Exact chunk directory. Use this if automatic discovery does not "
            "match the on-disk hierarchy."
        ),
    )
    parser.add_argument(
        "--similarity-cutoff",
...
def unique_existing_directories(candidates: Iterable[Path]) -> List[Path]:
    seen = set()
    found = []
...
def contains_any_expected_archive(directory: Path) -> bool:
    return any((directory / ARCHIVE_TEMPLATE.format(i)).is_file()
               for i in EXPECTED_SUBCHUNKS)


def resolve_chunk_directory(
    library_root: Path,
    chunk: int,
    explicit_chunk_dir: Optional[Path],
) -> Path:
    if explicit_chunk_dir is not None:
        if not explicit_chunk_dir.is_dir():
            raise FileNotFoundError(
                "--chunk-dir does not exist or is not a directory: {}".format(
                    explicit_chunk_dir
                )
            )
        return explicit_chunk_dir.resolve()
    ...
    chunk_strings = {
        ...
        "{:05d}".format(chunk),
        "chunk_{}".format(chunk),
        "chunk_{:05d}".format(chunk),
        "chunk{}".format(chunk),
        "chunk{:05d}".format(chunk),
    }
    superchunk_strings = {
        str(superchunk),
        "{:03d}".format(superchunk),
        "superchunk_{}".format(superchunk),
        "superchunk_{:03d}".format(superchunk),
        "superchunk{}".format(superchunk),
        "superchunk{:03d}".format(superchunk),
    }

    direct_candidates = [root / name for name in chunk_strings]
    for super_name in superchunk_strings:
        super_dir = root / super_name
        direct_candidates.extend(super_dir / chunk_name for chunk_name in chunk_strings)

    existing = unique_existing_directories(direct_candidates)
    archive_matches = [path for path in existing if contains_any_expected_archive(path)]

    if len(archive_matches) == 1:
        return archive_matches[0].resolve()
    if len(archive_matches) > 1:
        raise RuntimeError(
            "Multiple possible chunk directories contain expected archives:\n  {}".format(
                "\n  ".join(str(path) for path in archive_matches)
            )
        )

    # Limited fallback: identify plausible superchunk directories at the root,
    # then inspect only a few levels below them. This avoids recursively scanning
    # all 531 superchunks.
    plausible_superdirs = []
    for child in root.iterdir():
        ...
        if (
            child.name in superchunk_strings
            or normalized in {name.lower() for name in superchunk_strings}
            or normalized.endswith("_{}".format(superchunk))
            or normalized.endswith("_{:03d}".format(superchunk))
        ):
            plausible_superdirs.append(child)

    fallback_matches = []
    for super_dir in plausible_superdirs:
        for candidate in super_dir.glob("*"):
            if candidate.is_dir() and candidate.name in chunk_strings:
                if contains_any_expected_archive(candidate):
    ...
    fallback_matches = unique_existing_directories(fallback_matches)
    if len(fallback_matches) == 1:
        return fallback_matches[0].resolve()
    if len(fallback_matches) > 1:
        raise RuntimeError(
            "Multiple fallback chunk directories were found:\n  {}".format(
    ...
    raise FileNotFoundError(
        "Could not locate chunk {} (superchunk {}). Tried common paths:\n  {}\n"
        "Supply the exact directory with --chunk-dir.".format(
            chunk, superchunk, tried
        )
    )
...
def main() -> int:
    ...
    try:
        chunk_dir = resolve_chunk_directory(
            args.library_root, args.chunk, args.chunk_dir
        )
