...
def remove_tree_best_effort(path: Path) -> bool:
    """
    Remove a temporary directory without failing the pipeline on PanFS EBUSY.

    PanFS may temporarily replace an open-but-deleted file with a hidden
    .panfs.* file. Attempting to remove the directory while that file is still
    considered busy raises OSError(errno.EBUSY). In that case, leave the
    temporary directory in place and allow the successful outputs to complete.

    Returns True when the directory was removed or no longer exists, otherwise
    False.
    """
    if not path.exists():
        return True
    ...
    def onerror(function, failed_path, exc_info):
        ...
        if isinstance(exc, OSError) and exc.errno == errno.EBUSY:
            cleanup_errors.append((failed_path, exc))
            return
        raise exc
    ...
    if path.exists():
        busy_paths = []
        for failed_path, _ in cleanup_errors:
            busy_paths.append(str(failed_path))

        if not busy_paths:
            try:
                busy_paths = [
                    str(candidate)
                    for candidate in path.rglob(".panfs.*")
                ]
            except OSError:
                busy_paths = []

        log(
            "WARNING: temporary directory could not be fully removed because "
            "PanFS reports a device or resource as busy: {}".format(path)
        )
        if busy_paths:
            log(
                "WARNING: busy temporary path(s): {}".format(
                    ", ".join(busy_paths[:10])
                )
            )
        log(
            "WARNING: outputs will still be finalized. The temporary directory "
            "can be deleted later after PanFS releases the open handle."
        )
        return False
