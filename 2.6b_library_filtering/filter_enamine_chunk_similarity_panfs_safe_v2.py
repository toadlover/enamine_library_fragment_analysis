...
def remove_tree_best_effort(path: Path) -> bool:
    """
    Best-effort cleanup for PanFS temporary directories.

    PanFS may create hidden .panfs.* files when a deleted file still has an
    open handle. This can produce EBUSY during file deletion and ENOTEMPTY when
    removing the parent directory. Neither condition should fail the filtering
    job or prevent completed outputs from being finalized.

    Returns True if the directory no longer exists, otherwise False.
    """
    if not path.exists():
        return True
    ...
    def onerror(function, failed_path, exc_info):
        ...
        if isinstance(exc, OSError) and exc.errno in (
            ...
            cleanup_messages.append((str(failed_path), exc))
            return

        # Cleanup is deliberately nonfatal. Record unexpected cleanup errors
        # rather than allowing them to roll back otherwise successful outputs.
        cleanup_messages.append((str(failed_path), exc))

    try:
        shutil.rmtree(str(path), onerror=onerror)
    except OSError as exc:
        cleanup_messages.append((str(path), exc))
    ...
    busy_paths = []
    try:
        busy_paths = [str(candidate) for candidate in path.rglob(".panfs.*")]
    except Exception:
        busy_paths = []

    log(
        "WARNING: temporary cleanup was incomplete; this does not invalidate "
        "the filtering results: {}".format(path)
    )

    if busy_paths:
        log(
            "WARNING: PanFS temporary file(s) still present: {}".format(
                ", ".join(busy_paths[:10])
            )
        )
    elif cleanup_messages:
        examples = []
        for failed_path, exc in cleanup_messages[:5]:
            examples.append("{} ({})".format(failed_path, exc))
        log(
            "WARNING: cleanup issue(s): {}".format("; ".join(examples))
        )

    log(
        "WARNING: leaving the temporary directory for later cleanup after "
        "PanFS releases any open handles."
    )
    return False
    ...
    try:
        ...
        try:
                        ...
                        for sdf_member_path in extracted_sdfs:
                            ...
                            try:
                                ...
                                if exc.errno == errno.EBUSY:
                                    log(
                                        "WARNING: could not delete busy temporary "
                                        "file {}; cleanup will be retried later.".format(
                                            sdf_member_path
                                        )
                                    )
                                else:
            ...
            try:
                ...
                log(
                    "WARNING: unexpected temporary-cleanup error was ignored: "
                    "{}".format(cleanup_exc)
                )
