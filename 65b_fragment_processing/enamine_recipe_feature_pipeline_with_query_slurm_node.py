#!/usr/bin/env python3
"""
Full-node Slurm adapter for enamine_recipe_feature_pipeline_with_query.py.

This script keeps the original chemistry, feature extraction, binary formats,
manifest format, and query behavior. It adds a `node` mode that:

1. discovers input .bz2 files under one input directory,
2. creates/reuses a manifest,
3. runs several manifest tasks concurrently inside ONE Slurm allocation.

This avoids consuming one scheduler job per feature-extraction task. With a
QOS limit of four running jobs, four full-node jobs can each run multiple local
workers.
...
def batch_is_complete(output_root, task_id):
    """
    Treat a batch as complete only when its stats JSON exists and parses.

    This makes node mode restartable: completed tasks are skipped, while missing
    or partially written batches are rerun.
    """
    batch_name = "batch_{:06d}".format(task_id)
    batch_dir = Path(output_root) / batch_name
    stats_path = batch_dir / "{}.stats.json".format(batch_name)
...
def node_main(args, pipeline, adapter_path):
    ...
    if local_workers is None:
        env_cpus = os.environ.get("SLURM_CPUS_ON_NODE")
        local_workers = int(env_cpus) if env_cpus else 1
...
def build_parser(pipeline, default_pipeline_path):
    parser = argparse.ArgumentParser(
        description="Full-node Slurm adapter for Enamine feature extraction."
    )
    parser.add_argument(
        "--pipeline-script",
        default=str(default_pipeline_path),
        help="Path to the original pipeline Python file.",
    )
    ...
    node = subparsers.add_parser(
        "node",
        help="Create/reuse one manifest and run many tasks concurrently on one node.",
    )
    ...
    node.add_argument(
        ...
        default=None,
        help=(
            "Concurrent feature-extraction subprocesses inside this allocation. "
            "Default: SLURM_CPUS_ON_NODE, otherwise 1."
        ),
    )
    node.add_argument(
        "--python-executable",
        default=sys.executable,
        help="Python executable used for local worker subprocesses.",
    )
    node.add_argument(
        "--reuse-manifest",
        action="store_true",
        help="Reuse output-root/manifest.tsv and input_file_counts.tsv when present.",
    )
    node.add_argument(
        "--skip-completed",
        action="store_true",
        help="Skip batches with a valid batch_XXXXXX.stats.json file.",
    )
    node.add_argument(
        "--fail-fast",
        action="store_true",
        help="Stop scheduling new work after the first task failure.",
    )
